/****************************************************************************/
/* satz41.c + distinction of binary clauses and a further detection of      */
/*   contradiction                                                          */
/*                                                                          */
...
long NB_UNIT=0, NB_MONO=0, NB_BRANCHE=0, NB_BACK;
...
my_type build_sat_instance(int numvar, int numclause, int *wff) {
  ...
  int lits[1000], *plit, lit, NB_CLAUSE1, is_res;
...
int choose_and_instantiate_variable_in_clause() {
	...
	for (var=0; var<NB_VAR; var++)
      ...
      if (TESTED_VAR_STACK_fill_pointer < H_SEUIL)    {
	for (i=0; i<TESTED_VAR_STACK_fill_pointer; i++) {
	  var=TESTED_VAR_STACK[i];
	  if (var_state[var] == ACTIVE) {
	    insert_var_if_necessary1(var);
	  }
	}
	TESTED_VAR_STACK_fill_pointer=0; 
	for(pvar_node=VAR_FOR_TEST1; pvar_node!=NULL; 
	    pvar_node=pvar_node->next) {
	  var=pvar_node->var;
	  if (var_state[var]==ACTIVE) {
	    if (examine2(var) == NONE) {
	      saved_nb_back = NB_BACK;
			...
			int * soln, int argc, char * argv[] )
{
  ...
  case TOOBIG: 
    ...
    return Failure;
  case FALSE: 
    fprintf(STDMSG, "Input file error\n"); 
    exit(1);
  case TRUE:
    VARIABLE_STACK_fill_pointer=0;
    CLAUSE_STACK_fill_pointer = 0;
    ...
    break;
  case NONE: 
    fprintf(STDMSG, "An empty resolvant is found!\n"); 
    fprintf(STDMSG, "the instance is unsatisfiable ****\n");
    break;
  }
  
  if (satisfiable()) {
    fprintf (STDMSG, "****the instance is satisfiable *****\n");
    if (verify_solution()) {
      ...
      fprintf (STDMSG, "****PROGRAM ERROR, VERIFICATIOn FAILS****\n");
  }
  else { 
    if (answer == NONE)
  ...
  fprintf(fp_time, "satz211 %s %5.3f %ld %ld %ld %ld %d %d %d %d %ld %ld\n", 
  ...
  printf("satz211 %s %5.3f %ld %ld %ld %ld %d %d %d %d %ld %ld\n", 
          saved_input_file, ((double)(endtime-begintime)/CLK_TCK), 
          NB_BRANCHE, NB_BACK,  NB_SEARCH, NB_FIXED, 
          satisfiable(), NB_VAR, INIT_NB_CLAUSE, NB_CLAUSE-INIT_NB_CLAUSE,
          NB_SECOND_SEARCH, NB_SECOND_FIXED);
