...
/*************   newly added for randomization   *******************/
#define VERSION "satz-rand-214"
#define TOOBIG  -2
int branch();
int unitclause_process();
extern int bb_firstk;
int firstk;
int luby_factor;
long NB_OP, NB_NOOP;
...
struct timeval tv;
//struct timezone tzp;
float factor = 0.4;           	/* increment factor for chosen_var */
int cutoff;      		/* increment factor for chosen_var */
int restart;
int restart_max;
long x = 0;
...
double s = 0.0;
double sum_s = 0.0;
double sum_s_squared = 0.0;
double mean_s;
double variance_s;
double std_dev_s;
double std_error_mean_s;
/************************************************************/
...
double elapsed_seconds(void)
{
  ...
  clock_t tms;
  tms = clock();
  answer = (double)(tms-prev_times)/(double) CLOCKS_PER_SEC;
  prev_times = tms;
...
/******* newly added *******/
void save_solution(int nb_var, int * soln)
{
  int i;
  
  for (i=0; i<nb_var; i++){
    //saved_soln[i] = var_current_value[i];
    soln[i+1] = var_current_value[i];
  }
}
...
my_type build_sat_instance(int numvar, int numclause, int *wff) {
   char ch, tautologie, word2[WORD_LENGTH];
   int i, j, length, ii, jj, lit1, d,
     lits[1000], *plit, lit, NB_CLAUSE1, is_res;

   // if (fp_in==NULL) return FALSE;

  if (numvar > tab_variable_size ||
      numclause > tab_clause_size) return TOOBIG;
  NB_VAR = numvar;
  NB_CLAUSE = numclause;
...
int choose_and_instantiate_variable_in_clause() {
    ...
    int  i, posi, nega, num_good_vars;
    int nb_clauses, max_nb_clauses = -1; 
    my_type pos2, neg2;
    long saved_nb_back;
    struct var_node *pvar_node;
   int good_vars[tab_variable_size];
    ...
    if (bb_firstk > 0) {
     for (i=0; i<TESTED_VAR_STACK_fill_pointer; i++) {
       var=TESTED_VAR_STACK[i];
       if (var_state[var] == ACTIVE && var <= bb_firstk) {
          posi=reduce_if_positive_nb[var];
          nega=reduce_if_negative_nb[var];
          nb_clauses = posi*nega*1024 + posi + nega +1;
          if (nb_clauses >= max_nb_clauses) 
	 ...
	 nb_clauses = posi*nega*1024 + posi + nega +1;
	 if (max_nb_clauses == 0 || 
	     ((float)nb_clauses)/((float)max_nb_clauses) >= (1.0 - factor)){
	   good_vars[num_good_vars] = var;
	   num_good_vars++;
	 }	
       }
     }
   ...
   if (num_good_vars == 0) {
	 ...
	 if (max_nb_clauses == 0 || 
//       (var < bb_firstk &&
//	    ((float)nb_clauses)/((float)max_nb_clauses) >= (1.0 - opfactor)) ||
	     ((float)nb_clauses)/((float)max_nb_clauses) >= (1.0 - factor)){
	   good_vars[num_good_vars] = var;
	   num_good_vars++;
	 }	
   ...
   if (num_good_vars > 0) {
       chosen_var = good_vars[random()%num_good_vars];
       if (chosen_var <= bb_firstk)
	 NB_OP++;
       else
	 NB_NOOP++;
       //       total_choices ++;
       //sum_n += num_good_vars;
       //sum_n_squared += num_good_vars * num_good_vars;
       //mean_n = sum_n / total_choices;
   }
   if (chosen_var == NONE) return FALSE;
...
void print_statistics ()
{
  ...
  numsuccesstry ++;
  sum_s_squared += s * s;
  mean_s = sum_s / numsuccesstry;
  totalsuccessback += NB_BACK;
  integer_sum_x += x;
  ...
  */
  ...
  s = elapsed_seconds();
  fprintf(STDMSG, "[%d] %s %d %d %d %d %ld %ld %5.3f %d\n", restart,
	  VERSION, NB_BACK, NB_BRANCHE, NB_OP, NB_NOOP, NB_SEARCH, 
	  NB_FIXED, s, satisfiable());
...
int dpl() {
  ...
  for (restart = 1; restart <= restart_max; restart++) {
    
    if (luby_factor != 0)
      cutoff = luby_factor * luby_super(restart);
    reset_all();
    for(var=0; var<NB_VAR; var++) test_flag[var]=0;
    do {
      if (UNITCLAUSE_STACK_fill_pointer==0)
          if (choose_and_instantiate_variable_in_clause()==NONE)
              backtracking();
      if (unitclause_process()==NONE) backtracking();
      if (bb_limitp(Satz_Rate)){
      	return NONE;
      }
      if (NB_BACK >= cutoff) break;
    } while ((VARIABLE_STACK_fill_pointer != 0) && (!(satisfiable())));
    print_statistics();
    if (satisfiable()) return TRUE;
    if (NB_BACK != cutoff && VARIABLE_STACK_fill_pointer == 0) {
      return FALSE;
    }
  }
  return NONE;
...
void init_satz (void)
{
  CLAUSE_NODES_pointer=MAX_NODE_NUMBER;
  cutoff = 1000000000;
  restart = 0;
  restart_max = 1;
}
...
int bb_satsolve_satz( int numvar, int numclause, int * wff, int * soln, 
                      int maxtime, int argc, char * argv[] )
{
  ...
  init_satz();
  fprintf(STDMSG, "Invoking solver satz-rand version %s\n", VERSION);
  struct timezone tzp;
  gettimeofday(&tv,&tzp);
  ...
  while (++i < argc){
    ...
    //	    numsol = atoi(argv[++i]);
      ...
      restart_max = atoi(argv[++i]);
    //else if (strcmp(argv[i],"-printsol")==0)
    //	    nosol = FALSE;
    else if (strcmp(argv[i], "-nofirstk") == 0)
    	    firstk = 0;
    else {
      fprintf(STDMSG, "Options for satz (satz-rand) solver are:\n"
	      "    -seed N = initialize random seed to N\n"
	      "    -cutoff N = stop search after N backtracks, and then (possibly) restart\n"
              "    -cutoff luby K = cutoff is equal to K * luby series\n"
	      "    -restart N = restart up to N times or until problem is solved\n"
	      "    -numsol N = keep going until the problem is solved N times (default 1)\n"
	      "    -noise F = degrade variable choice heuristic to increase randomness\n"
	      "               between restarts: 1.0 = random, 0.0 = no noise,\n"
	      "               default = 0.4 if restart > 1, otherwise default = 0.0\n"
	      "    -nofirstk don't branch only on the operator variables\n");
      exit(1);
    }
  }

  /* Should the factor be 0 if there are no restarts? 
     Seems to HURT on some wffs! */
  if (factor < 0.0){
    if (restart_max == 1) 
      factor = 0.0; /* Maybe make factor = 0.0 in this case */
    else 
      factor = 0.4;
  }
  ...
  fprintf(STDMSG, "seed: %d\ncutoff: %d\nnoise: %f\nrestart: %d\n", seed, cutoff, factor, restart_max);
  fprintf(STDMSG, "fields: NB_BACK NB_BRANCH NB_OP NB_NOOP NB_SEARCH time sat\n");
   ...
   switch (build_sat_instance(numvar, numclause, wff)) {
   case TOOBIG: 
     fprintf(STDMSG, "satz-rand: wff too big, must recompile!\n");
     return Failure;
   ...
   case NONE: 	
     ...
     fprintf(STDMSG, "the instance is unsatisfiable ****\n");
     break;
   }
#ifndef WIN32
   ...
   if (satisfiable()) {
     ...
     if (verify_solution()) {
        fprintf (STDMSG, "****verification of solution is OK****\n");
        // print_values(NB_VAR);
	save_solution(NB_VAR, soln);
	answer = Sat;
     }
     else
    ...
    if (answer == NONE)
      fprintf(STDMSG, "****Reached Blackbox resource limits\n");
    else
      fprintf (STDMSG, "****the instance is unsatisfiable *****\n");
    answer = Unsat;
  }
  ...
  fprintf (STDMSG, "Program terminated in %5.3f seconds.\n",
