...
/*************************************************************************
  ...
  3/20/98 - I turned off the completeness check for non-graphplan,
  and for Timeout graphplan -- seems to terminate too early.
...
extern fact_list initial_facts, the_goals;
extern char *problem_name;
extern char *outputfile;
extern FILE *learnfp;
extern int bb_stop;
extern struct timeval start, end;
extern int noskip_flag, start_sat_solver, find_opt;
...
void print_something(int maxtime)
{
  extern vertex_t * bb_prop2vertex;
  FILE *foo;
  ...
  for (int i = 1; i <= bb_numvar; i++)
    bb_soln[i] = 0;
  for (int i = 0; i < bb_numclause; i++) {
    if (bb_wff[j + 1] == 0) {
      k++;
      l = abs(bb_wff[j]);
      bb_soln[l] = (bb_wff[j] > 0) ? 1 : -1;
    }
    for (; bb_wff[j] != 0; j++);
    j++; 
  }
  ...
  for(int i=0; i < maxtime; i++) {
    ...
    while((v = get_next(op_table[i],1)) != NULL) {
      if (v->needed != 0)
	k++;
      if (v->needed != 0 && v->is_true == 0)
	fprintf(foo, "%d %s\n", i, v->name);
    }  
  }  

  for(int i=0; i < maxtime; i++) {
    get_next(fact_table[i],0);
    while((v = get_next(fact_table[i],1)) != NULL) {
      if (v->needed != 0)
	k++;
      if (v->needed != 0 && v->is_true == 0)
	fprintf(foo, "%d %s\n", i, v->name);
...
/* process the planning graph to make the facts via noop is true */
void
process_graph_nodes (void)
{
  int i;
//  fact_list f;
  vertex_t v;
...
//  char str[50];
  ...
  for (f = initial_facts; f; f = f->next) {
    instantiate_into_string(f->item, NULL, str, 0);
    if ((v = lookup_from_table(fact_table[0], str)))
      v->is_true++;
  }
  */
}
...
int do_plan(int maxtime)
{
  ...
  fprintf(STDMSG,"\n####################################################\n");
  fprintf(STDMSG,"goals at time %d:\n  ",maxtime+1);
  for(i=0; i < num_goals; ++i) { 
      fprintf(STDMSG,"%s ",goals_at[maxtime][i]->name);
      if (((i+1)%5)==0) fprintf(STDMSG, "\n");
  }
  fprintf(STDMSG,"\n\n");
  ...
  for (i=0; i < bb_spec_len && (result == Timeout || result == Failure || 
				result == Simplified) ; i++){
      bb_init_limitp(bb_spec[i].maxsec, bb_spec[i].maxit);

      fprintf(STDMSG,"\n----------------------------------------------------\n");

      if (bb_spec[i].solver == Graphplan){
	  if (ran_gp) num_goals = setup(maxtime);

	  bb_reparse_graphplan_options(bb_spec[i].argc, bb_spec[i].argv);

	  fprintf(STDMSG, "Invoking solver graphplan\n");
	  if (do_buildup) result = build_up_to_plan(num_goals, maxtime);
	  else if (do_greedy) result = constrict(num_goals, maxtime); 
	  else result = runbasicplan(num_goals, maxtime);

	  /* Following terminates prematurely if done for non-graphplan solvers,
	     or for graphplan when it timesout */

	  if (result == Unsat && do_memo) insert_bad(goals_at[maxtime], num_goals, maxtime);
	  if (result == Timeout && noskip_flag) {
	    for (j = i; j < bb_spec_len - 1; j++) {
	      bb_spec[j].solver = bb_spec[j + 1].solver;
	      bb_spec[j].maxsec = bb_spec[j + 1].maxsec;
	      bb_spec[j].maxit = bb_spec[j + 1].maxit ;
	      bb_spec[j].argc = bb_spec[j + 1].argc;
	      bb_spec[j].argv = bb_spec[j + 1].argv;
	      bb_spec[j].proc = bb_spec[j + 1].proc; 
	    }
	    bb_spec_len--;
	    i--;
	  }
	  if (result == Timeout && do_completeness){
	      do_completeness = FALSE;
	      fprintf(STDMSG,"** Turning off completeness check **\n");
	  }
      }
      else if (bb_spec[i].solver == Compact){
          if (do_completeness){
              do_completeness = FALSE;
              fprintf(STDMSG,"** Turning off completeness check **\n");
          }
	  if (! made_wff) {
	      bb_graph2wff(maxtime, num_goals);
	      made_wff = TRUE;
	  }
	  result = bb_simplifier_compact (bb_numvar, bb_numclause, bb_wff,
					 &bb_numclause, bb_spec[i].maxsec,
                                          bb_spec[i].argc, bb_spec[i].argv);
      
	 //// print_something(maxtime); 
      }
      else {
	  /* Have to turn off completeness when not using graphplan */
	  if (do_completeness){
	      do_completeness = FALSE;
	      fprintf(STDMSG,"** Turning off completeness check **\n");
	  }
	  ...
	  result = (* (bb_spec[i].proc))(bb_numvar, bb_numclause, bb_wff, 
					 bb_soln, bb_spec[i].maxsec,
					 bb_spec[i].argc, bb_spec[i].argv);
	  if (!start_sat_solver && result != Sat)
	    start_sat_solver = 1;
	  if (result == Sat) 
	    bb_soln2graph();
      }
      fprintf(STDMSG, "Result is %s\n", bb_return_val_names[result]);
      fprintf(STDMSG, "Iteration was %ld\n", bb_current_it);
      if (result == Failure) something_failed = TRUE;
      if (bb_stop) {
	result = Timeout;
	break;
      }
  ...
  if (result != Sat && xviewing == 1) { /* clean up the viewer */
    for(i=0; i < num_goals; ++i) {
  ...
  if (result == Sat) {
    ...
    process_graph_nodes();
#endif
    justify_plan(maxtime);
  } 
  /* return result; */
  /* return result == Sat; */
  if (result == Sat || result == Unsat) return result;
  if (something_failed) return Failure;
  return result;
}
...
/* print one operation               */
/*************************************/
void print_op (FILE * outputfp, char *name)
{
    fputc('(', outputfp);
    while (*name) {
	if (*name == '_')
...
void print_plan(int maxtime)
{
	...
	while((v = get_next(op_table[i],1)) != NULL) {
		...
		if (bb_prettyflag){
		    fprintf(STDDATA,"%d ",i+1);
		    print_op(STDDATA, v->name);
		    fprintf(STDDATA,"\n");
		}
		else
		    fprintf(STDDATA,"%d %s\n",i+1, v->name);
    ...
    fprintf(STDDATA, "End plan\n");
    fprintf(STDMSG,"----------------------------------------------------\n\n");
    fprintf(STDMSG, "%d total actions in plan\n", actcount);
    if (do_memo) { /* print out some useful info */
	fprintf(STDMSG,"%d entries in hash table, ",hash_inserts);
	if (hash_inserts == 0) fprintf(STDMSG,"\n");
	else fprintf(STDMSG,"%d hash hits, avg set size %d\n",
		     hash_hits, hash_numctr/hash_ctr);
	fprintf(STDMSG,"%d total set-creation steps (entries + hits + plan length - 1)\n",
		hash_inserts + hash_hits + maxtime - 1);
    }
    if (do_subsets) fprintf(STDMSG,"%d subset hits.\n", subset_hits);
    if (do_greedy) fprintf(STDMSG,"%d hits on doable sets\n", goodhash_hits);
    fprintf(STDMSG,"%d actions tried\n", number_of_actions_tried);
    /*
    if (outputfile){
    ...
    */
    ...
    if (learn_mode) {
      ...
      if (learn_mode & VERIFY_CONTROL)
	verify(1);
      if (learn_mode & LEARN_CONTROL)
	learn();
      print_paren_end();
      fclose(learnfp);
      /*
...
int basicplan(int cindex, int nindex, int time)
{
  ...
  if (bb_limitp(Graphplan_Rate)) return Timeout;


  if (cindex < 0) { /* go on to the previous time step */
    if (DEBUG_FLAG) {
      fprintf(STDMSG,"goals at time %d:\n  ",time);
      for(i=0; i < nindex; ++i) fprintf(STDMSG,"%s ",goals_at[time-1][i]->name);
      fprintf(STDMSG,"\n\n");
    }
    if (do_memo) {
    ...
    if (!action_set_is_minimal(nindex, time-1)) {
      fprintf(STDMSG,"found non-minimal action set, time %d\n",time);
      return 0;
    }
    ...
    if (result == Unsat && do_memo) insert_bad(goals_at[time-1], nindex, time-1);
    return result;
  }
  ...
  for(e = v->in_edges; e; e = e->next) {
    ...
    /* now recurse */
    if (DEBUG_FLAG > 1) {
      if (IS_NOOP(op)) fprintf(STDMSG,"trying %s from prev time step.\n",v->name);
      else fprintf(STDMSG,"Trying %d %s to achieve %s.\n",time, op->name, v->name);
    }
...
int can_hope_to_solve(goal_arr g, int length, int time)
{
  ...
  if (yesind > time) {
    if (DEBUG_FLAG) fprintf(STDMSG,"Can't hope to solve by time %d.\n",time);
    return 0;
  }
  else return 1;
}
...
int build_up_to_plan(int num_goals, int time)
{
  ...
  for(;i < num_goals; ++i) {
    if (i != num_goals-1) {
      fprintf(STDMSG,"trying first %d goals\n",i+1);
      for_printing = 0;
    ...
    if (success != Sat) { 
      ...
      if (success == Unsat && do_memo) insert_bad(goals_at[time], num_goals, time);
      for_printing = oldfp;
      return success;
    }
  }
...
int constrict(int num_goals, int time)
{
  ...
  /* if (runbasicplan(num_goals, time)) return 1; */
  result = runbasicplan(num_goals, time);

  if (result == Sat || result == Timeout) return result;
...
int lookup_bad(goal_arr a, int num, int time)
{
  ...
  if (val == 1) {
    if (DEBUG_FLAG) fprintf(STDMSG,"hash table hit.\n");
...
int lookup_good(goal_arr a, int num, int time)
{
  ...
  if (val == 1) {
    if (DEBUG_FLAG) fprintf(STDMSG,"good hash table hit.\n");
...
hash_entry set_insert(set_table table, goal_arr a, int num, int time)
{
  ...
  if (elt == NULL) { /* out of memory */
    if (mem_flag == 0)
      ++mem_flag, fprintf(STDMSG,"no memory at insert %d\n",hash_inserts);
    return NULL;
  }
  elt->num = num;
  elt->sum = sum;
...
void insert_bad(goal_arr a, int num, int time)
{
  ...
  if (do_completeness && same_as_prev_flag) { /* info for completeness test */ 
	...
	/* SHOULD NEVER HAPPEN */
	fprintf(STDMSG,"Error! set in table at time %d not in at time %d.\n",
	       first_full_time+1, first_full_time);
	for(i=0;i<num;++i) fprintf(STDMSG,"%s ",a[i]->name);
	fprintf(STDMSG,"\n");
	exit(1);
      }
    }
...
int setup(int maxtime)
{
  ...
  /* newly added for finding the optimal solution backward */
  if (find_opt) {
    get_next(op_table[maxtime],0);                  
    while((v = get_next(op_table[maxtime],1)) != NULL)
      v->needed = 0;
    get_next(fact_table[maxtime],0);               
    while((v = get_next(fact_table[maxtime],1)) != NULL)
      v->needed = 0;
  }
