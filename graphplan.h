...
#ifndef __GRAPHPLAN_H_
#define __GRAPHPLAN_H_
...
#define MAXMAXNODES (32*1024)  /* max number of nodes per layer of graph */
#define NUMINTS (MAXMAXNODES/32) /* MAXMAXNODES / 32  ... 1024 gives 32 */

#define DEFAULT_MAXNODES (2*1024)
...
struct VERTEX 
{
  ...
  int prop; /* proposition assigned to this vertex in wff form */
  struct VERTEX *next;
};
...
typedef struct FACTLIST {
  token_list item;
  struct FACTLIST *body;  /* added for handling exists */
...
element_s * lookup_token_list(hashtable_t htable, token_list t);
...
extern goal_arr *goals_at;

extern int do_subsets;
extern int do_irrel;
extern int do_buildup;
extern int do_greedy;
extern int do_lower;
extern int do_noexcl;
extern int do_completeness;
