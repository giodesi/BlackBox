...
/* Written by: Yi-Cheng Huang at AT&T 1999 Summer */
	       ...
	       vertex_t act, int st_time)
{
  ...
  vertex_t v_temp, next_fact, next_act, tmp;
  ...
  for (e_fact = act->out_edges; e_fact; e_fact = e_fact->next) {
    ...
    next_fact->junk = PRUNED;	/* really prune fact */
    next_fact->is_true = 0;
...
restore_action (edgelist_t *pruned_act, edgelist_t *changed_fact, int maxtime)
{
  ...
  for (i = 0; i <= maxtime; i++) {
    ...
    for (e = changed_fact[i]; e; e = e->next) {
      e->endpt->is_true = (e->endpt->junk == PRUNED) ? 1 : 0;
      e->endpt->junk = 1;	
    }
  }
}
